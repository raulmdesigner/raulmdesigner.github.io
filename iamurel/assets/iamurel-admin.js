document.addEventListener("DOMContentLoaded", () => {
    ...
    async function loadSettingsAdmin() {
        // Usando maybeSingle para evitar travamento se a tabela estiver vazia
    ...
    document.getElementById('form-appearance').addEventListener('submit', async (e) => {
        ...
        const { data: settings } = await iamurelSupabase.from('iamurel_site_settings').select('id').limit(1).maybeSingle();
        ...
        if (settings) {
            ...
            const { error } = await iamurelSupabase.from('iamurel_site_settings').insert([updates]);
            if (error) { 
                msg.textContent = 'Falha ao criar configuração. Verifique as permissões RLS.'; 
                msg.classList.add('text-red-500'); 
            } else { 
                msg.textContent = 'Configurações criadas e ativas!'; 
                msg.classList.add('text-green-600'); 
            }
        }
    });
